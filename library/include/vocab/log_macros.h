...
#define L_INF(...)                                                        \
  {                                                                       \
    std::clog << "INF " << __FILE__ << ':' << std::dec << __LINE__ << ' ' \
              << __VA_ARGS__ << '\n';                                     \
  }
#define L_WRN(...)                                                        \
  {                                                                       \
    std::clog << "WRN " << __FILE__ << ':' << std::dec << __LINE__ << ' ' \
              << __VA_ARGS__ << '\n';                                     \
  }
#define L_ERR(...)                                                        \
  {                                                                       \
    std::clog << "ERR " << __FILE__ << ':' << std::dec << __LINE__ << ' ' \
              << __VA_ARGS__ << '\n';                                     \
  }
#define L_DIE(...)                                                        \
  {                                                                       \
    std::clog << "Fatal error! Aborting! " << __FILE__ << ':' << std::dec \
