...
/*
using libp2p::crypto::KeyValidator;

KeyValidator::KeyValidator(std::shared_ptr<CryptoProvider> crypto_provider)
    : crypto_provider_{std::move(crypto_provider)} {}

outcome::result<void> KeyValidator::validate(const PrivateKey& key) const {
  switch (key.type) {
...
outcome::result<void> KeyValidator::validate(const PublicKey& key) const {
  switch (key.type) {
    case Key::Type::UNSPECIFIED:  // consider unspecified key valid
      return outcome::success();
    case Key::Type::RSA:
...
outcome::result<void> KeyValidator::validate(const KeyPair& keys) const {
...
outcome::result<void> KeyValidator::validateRsa(const PrivateKey& key) const {
...
outcome::result<void> KeyValidator::validateRsa(const PublicKey& key) const {
  const unsigned char* data_pointer = key.data.data();
...
outcome::result<void> KeyValidator::validateEd25519(
...
outcome::result<void> KeyValidator::validateSecp256k1(
    const PrivateKey& key) const {
...
outcome::result<void> KeyValidator::validateEcdsa(const PrivateKey& key) const {
  // TODO(xDimon): Check if it possible to validate ECDSA key by some way.
  //  issue: https://github.com/libp2p/cpp-libp2p/issues/103
  return outcome::success();
}

outcome::result<void> KeyValidator::validateEcdsa(const PublicKey& key) const {
  // TODO(xDimon): Check if it possible to validate ECDSA key by some way.
  //  issue: https://github.com/libp2p/cpp-libp2p/issues/103
  return outcome::success();
}
